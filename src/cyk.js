export default function init() {
          ...
          for (var leftRootIndx in leftSubtreeRoots) {
            for (var rightRootIndx in rightSubtreeRoots) {
              var rls =
                grammar[
                  makeKey([
                    leftSubtreeRoots[leftRootIndx]["rule"],
  ...
  // перевожу грамматику в Хэш таблицу для быстрого доступа в ячейках
  ...
  function traverseParseTable(parseTable, left, right, rootIndex) {
    if (!parseTable[left][right][rootIndex]["middle"]) {
      return (
        '<li><a href="">' +
        parseTable[left][right][rootIndex]["rule"] +
        '</a><ul><li><a href="">' +
        parseTable[left][right][rootIndex]["token"] +
        "</a></li></ul></li>"
      );
    }
    return (
  ...
  //     "PowOp -> ^",
  //   ];
  // var parseTable = parse(
  //   grammarToHashMap(grammar),
  //   "Variable ^ Number + Number * Variable".split(" ")
  // );
  ...
  // пример взял с вики - http://en.wikipedia.org/wiki/CYK_algorithm#Example
  let grammar = [
    "S -> NP VP",
    "VP -> VP PP | V NP | eats",
    "PP -> P NP",
